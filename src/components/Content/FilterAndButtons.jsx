...
import ExtraFilterProduct from '../../common/ExtraFilterProduct';
...
export const Filters = ({ isHideSort }) => {
    ...
    return (
        <div style={{ display: 'flex', marginTop: '16px' }}>
            <SortingProduct isHide={isHideSort} />
            {!productId && <ExtraFilterProduct />}
...
export default function Content() {
    ...
    const handleOrderPlacedClose = (event, reason) => {
        ...
        dispatch({ type: 'orderPlacedFalse' });
    };

    const handleProductDeletedClose = () => {
        setRerender(true);
        dispatch({ type: 'productDeleted', payload: '' });
    };
