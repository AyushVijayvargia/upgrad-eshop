...
export default function Login(){
    ...
    const [openUsernamePasswordFailure,setOpenUsernamePasswordFailure] = useState(false);
    ...
    const handleSubmit = async (event) => {
     ...
     // api call
        const userData = await fetch('http://localhost:8080/api/auth/signin', {
          method: 'post',
     ...
     // local data set up
      const localuserData = JSON.parse(localStorage.getItem(formData.email));
       ...
       if(userData.status===200)
       {
        ...
        if(localuserData==null) {
          if(userData.status===200)
          {
            //what if we didnt signup ,but data was there in db
          //  localStorage.setItem(formData.email,JSON.stringify(formData));
            if(localStorage.getItem('admin@upgrad.com') === null) {
              ...
              localStorage.setItem('admin@upgrad.com', JSON.stringify(adminDetails));
               ...
               dispatch({type: 'login', payload: adminDetails});
            }
        ...
        if(localuserData.password !== formData.password) {
          setOpenPasswordFailure(true);
          return;
        }
        dispatch({type: 'login', payload: localuserData});
      };
      ...
      return (
        <ThemeProvider theme={theme}>
          ...
          <Snackbar open={openUsernamePasswordFailure} autoHideDuration={6000}>
            <Alert severity="error" sx={{ width: '100%' }}>
              The username/password that is entered is wrong
            </Alert>
          </Snackbar>
